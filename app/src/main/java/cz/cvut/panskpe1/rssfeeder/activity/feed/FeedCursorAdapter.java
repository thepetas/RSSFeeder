...
public class FeedCursorAdapter extends CursorAdapter {
    ...
    private void deleteFeedWithArticles(String id) {
        mContext.getContentResolver().delete(Uri.withAppendedPath(RssFeederContentProvider.CONTENT_URI_FEED, id), null, null);
        mContext.getContentResolver().delete(RssFeederContentProvider.CONTENT_URI_ARTICLE, FEED_ID + "= ?",
                new String[]{id});
    }
